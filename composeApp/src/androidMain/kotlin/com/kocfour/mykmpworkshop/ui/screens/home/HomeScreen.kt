...
@Composable
fun HomeScreen(mainNavHostController: NavHostController?=null) {
    ...
    Scaffold(bottomBar = {
        ...
        NavHost(navController, startDestination = NavItem.Home.path,Modifier.padding(innerPadding)) {
            composable(NavItem.Home.path) { HomeTab(mainNavHostController) }
            composable(NavItem.Search.path) { AnalysisTab() }
            composable(NavItem.List.path) { SettingsTab() }
            composable(NavItem.Profile.path) { ProfileTab(mainNavHostController) }
        }
    }
    }
}
